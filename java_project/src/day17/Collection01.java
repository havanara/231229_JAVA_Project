...
public class Collection01 {
	...
	public static void main(String[] args) {
		 ...
		 * List, Set, Map -> 인터페이스(반드시 구현한 클래스로 객체를 생성)
		 * 데이터를 묶어서 관리할 때 상ㅇ
		 * 주로 배열 대신 사용
		 * 
		 * List(배열과 동일)
		 * 값을 하나씩 저장
		 * 순서를 보장(index 0번지 1번지 ...)
		 * 중복 허용
		 * 배열 대신 가장 많이 사용하는 구조
		 * 
		 * Set
		 ...
		 * 
		 * Map
		 * 값을 두개씩 저장 Key / Value 한쌍으로 저장
		 * Key 중복 불가능 / Value 중복 가능 ex)Key 이름은? Value 홍길동
		 * Key가 중복되면 Value는 덮어쓰게 됨
		 * 아이디/패스워드와 같이 하나의 자료를 묶어서 저장해야 할 때 사용
		 * 
		 * 배열은 기본 자료형, 클래스 자료형으로 구성 가능
		 * int arr[], String arr[], Student arr[] ... 다 가능
		 * 
		 * Collection에서는 기본 자료형은 불가능함
		 * 클래스 자료형만 구성 가능
		...
		list.add(2);
		list.add(3);
		System.out.println(list); //list 내부에 toString을 가지고 있음
		
		System.out.println(list.get(0)); //list의 0번지 값을 가져옴
		//get(index) : index 번지의 요소를 가져옴

		list.set(0, 5);
		//set(index, 값) : index 번지의 값을 setting 
		...
		System.out.println();
		System.out.print("총개수:"+st.size()+"개");
		...
		//list1의 0번지 값을 7로 변경 후 출력 -> set(index,값)
		list1.set(0, 7);
		...
		//isEmpty() : 리스트가 비어져있는지 체크 -> true, false로 리턴
		 ...
		 * map은 향상된 for, Iterator 둘 다 직접 사용 불가
		 */
		...
		//indexOf(값) : 해당 값이 list에 확인 후 index 반환 / 없다면 -1 리턴
		...
		System.out.println(number);
		System.out.printf("-------sort(Comparator를 구현한 객체)-------%n");
		
		//sort(n->n.getName()를 구현한 객체) : 정렬
		...
		number.sort(new Comparator<Integer>() {
			...
			public int compare(Integer o1, Integer o2) {
				//number를 기준으로 o1은 앞에 있는 값 o2는 뒤에 있는 값

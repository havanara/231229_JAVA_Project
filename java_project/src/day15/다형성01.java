 ...
 * 각각의 객체는 메시지를 주고(리턴타입) 받으면서(매개변수) 데이터를 처리
 * 추상화, 캡슐화(정보은닉), 상속, 다형성
...
public class 다형성01 {
	...
	public static void main(String[] args) {
		...
		System.out.printf("%n---조상의 객체로 자식 객체 생성---%n");
		
//		Human h1 = new Animal(); -> 자식의 객체로 조상을 생성하는 것은 불가능
		
		//조상의 클래스로 자식의 객체 생성 가능
		//조상의 클래스에서 상속 받은것만 실행 가능
		//본인것은 실행 불가
		...
		System.out.printf("%n---다형성 메서드 예시---%n");
...
//		ex1.moveAnimal3(e);
		...
		Dog d = new Dog();
		d.move();
//		ex1.moveAnimal(d); -> animal 클래스를 상속받지 않고 때문에 불가능
		
		System.out.printf("%n---Animal 배열 생성 예시---%n");
		...
		System.out.printf("%n---다운 캐스팅 예시---%n");
		//휴먼을 담을 수 있는 자료형의 객체에 Animal을 상속받은 Human 클래스 객체를 담은 케이스 -> 다운캐스팅
		Human human = (Human)hAnimal;
		human.readBook();
		
		//ClassCastException 다운캐스팅의 잘못된 예시(바로 에러는 안나지만 실행하면 에러남)
//		Human human1 = (Human)tAnimal;
//		human1.readBook();
		
		System.out.printf("%n---instanceof---%n");
